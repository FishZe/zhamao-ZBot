...
class Sys{
	...
	public function filter(){
        ...
        if(in_array(substr($msg, 0, 1), array('#', '$'))){
            ...
            if($superList == null){ctx() -> reply("您还没有设置SuperQQ!"); return false;}
    ...
    public function getSuper(){
        if(ctx() -> getMessageType() == "group"){return "请私聊获得权限!"; }
        $qqId = ctx() -> getUserId();
        ...
        if($r != $uniqId){return "验证码错误！请重新验证！";}
        SpinLock::lock("SUPER_QQ");
        $superList = LightCache::get("SUPER_QQ");
        if($superList != null){ array_push($superList, $qqId); }
        else {$superList = array($qqId);}
        LightCache::set("SUPER_QQ", $superList);
        if(count($superList) == 1){ LightCache::addPersistence("SUPER_QQ"); }
